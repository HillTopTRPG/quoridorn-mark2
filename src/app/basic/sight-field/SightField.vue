...
export default class SightField extends PieceMixin {
  ...
  private paint(this: any) {
    ...
    let useDistance = this.distance * this.mapGridSize - 1;
    ...
    obstacleList.forEach((obstacle: any) => {
       ...
       * 基本座標算出
       */
      const width: number = obstacle.columns * this.mapGridSize;
      const height: number = obstacle.rows * this.mapGridSize;
  ...
  get tableSize(this: any): any {
    return {
      w:
        (this.mapMarginGridSize * 2 + this.mapColumns) * this.mapMarginGridSize,
      h: (this.mapMarginGridSize * 2 + this.rows) * this.mapMarginGridSize
    };
  }
  get centerX(): number {
